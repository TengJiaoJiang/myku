...
uint32_t TRACKING(MmwDemo_DSS_DataPathObj *obj, float CarrierVel, float CarrierAng)
{
	...
	for (i = 0; i < OncePointNumberClearn; i++)
	{
		...
		{
			if (OnceDataArr[i].v * 3.6 < 0 && OnceDataArr[i].y < 0 && OnceDataArr[i].v * 3.6 > -40.0)
			{
				fprintf(fWrite, "__ (v(km/h),x,y) %f %f %f￥￥￥￥￥￥￥￥￥\n", OnceDataArr[i].v * 3.6, OnceDataArr[i].x, OnceDataArr[i].y);
				VBSDbj = VBSDbj + 1;
	...
	fprintf(fWrite, "^^^^^^^^^ y<0 v<0 num = %d, BSDWARING_obj->iysmal0 = %d, lastnum = %d \n", VBSDbjysml0, BSDWARING_obj->iysmal0, LastUpdataNum(12, BSDWARING_obj->iysmal0));
    ...
    if (LastUpdataNum(12, BSDWARING_obj->TraceState) >= 5 && LastUpdataNum(12, BSDWARING_obj->iysmal0) > 1) // 5/4        8/6
	{
    ...
    if (LastUpdataNum(10, BSDWARING_ccobj->TraceState) >= 4) // 5/3
	{
